...
# AUDIO SYNC SETTINGS - Adjust these to sync lyrics with music
AUDIO_START_DELAY = 0.5  # Delay after countdown before first lyric (adjust this!)
TIMING_MULTIPLIER = 0.6  # Multiply all delays by this (1.0 = normal, 1.2 = 20% slower, 0.8 = 20% faster)

# Helper functions that apply timing multiplier
def wait(seconds):
...
while Complite == False:
    ...
    play.start()
    print(f"START !!!")
    time.sleep(AUDIO_START_DELAY)  # Adjust AUDIO_START_DELAY at top of file to sync
    power_line = False
    print_sync(style.RED+"Switch on the power line"+style.RESET, 0.03)
    power_line = True
    ...
    print_sync(style.RED+"Lay down your pieces"+style.RESET, 0.02)
    World.layDown()
    print_sync(style.RED+"And let's begin"+style.RESET, 0.02)
    print_sync(style.GREEN+" OBJECT_CREATION"+style.RESET, 0.02)
    data["objectCreation"] = True
    print("\n>>> object Creation : "+str(data["objectCreation"]))
    wait(0.5)
    print_sync(style.GREEN+"\n Fill in my data"+style.RESET, 0.04)
    print_sync(style.GREEN+" PARAMETER"+style.RESET, 0.0004)
    World.INITIALIZATION()
    data["Parameter"] = "100%"
    wait(0.2)
    print_sync(style.RED+"\n Set up a new World"+style.RESET, 0.03)
    ...
    if data["objectCreation"] == True and data["Parameter"] == "100%" and data["NewWorld"] == "setup done":
        ...
        if data["setOfPoint"] != "":
            ...
            print_sync(style.RED+" my"+style.RESET, 0.04)
            wait(0.1)
            print_sync(style.YELLOW+" DIMENSION"+style.RESET, 0.01)
            ...
            if data["Circel"] == len(data["setOfPoint"]):
                ...
                if len(data["sineWave"]) == 11:
                    ...
                    if data["im"] == "INFINITY":
                        ...
                        print("\n\n")
                        print_sync(style.RED+"Switch my "+style.RESET, 0.04)
                        print_sync(style.GREEN+"gender"+style.RESET, 0.02)
                        wait(0.2)
                        print_sync(style.GREEN+"\nTo ----------- > F"+style.RESET, 0.02)
                        data["you"].append("GANDER ::: FEMALE")
                        print(style.RED+"\n>>>SYSTEM SAYS : SUCCESS SET TO FEMALE !!"+style.RESET)
                        wait(0.1)
                        print_sync(style.GREEN+" ----------- > M"+style.RESET, 0.02)
                        data["you"].remove("GANDER ::: FEMALE")
                        data["you"].append("GANDER ::: MALE")
                        print(style.RED+"\n>>>SYSTEM SAYS : SUCCESS SWITCH TO MALE !!"+style.RESET)
                        print_sync(style.RED+"And then do "+style.RESET, 0.04)
                        print_sync(style.GREEN+"whatever"+style.RESET, 0.02)
                        ...
                        print_sync(style.RED+"I will switch "+style.RESET, 0.04)
                        print_sync(style.GREEN+"my role"+style.RESET, 0.02)
                        ...
                        wait(0.3)
                        print_sync(style.RED+"\nSo we can enter "+style.RESET, 0.04)
                        print_sync(style.GREEN+"the trance,"+style.RESET, 0.04)
                        print_sync(style.GREEN+"the trance"+style.RESET, 0.04)
                        wait(0.5)
                        World.EnterTrance("World.Execute.me.py")
                        print(style.MAGENTA+"\nIF I CAN")
                        wait(1.0)
                        print_sync(style.MAGENTA+"IF I CAN "+style.RESET, 0)
                        print_sync(style.MAGENTA+" feel your"+style.RESET, 0.04)
                        print_sync(style.YELLOW+" VIBRATIONS"+style.RESET, 0.02)
                        wait(1.0)
                        print(style.BLUE+"\nThen I can")
                        time.sleep(1)
